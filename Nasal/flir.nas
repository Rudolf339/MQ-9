...
var coords_cam = [
    getprop("/sim/view[100]/config/z-offset-m"),
    getprop("/sim/view[100]/config/x-offset-m"),
    getprop("/sim/view[100]/config/y-offset-m")
];
...
#        }
    ...
    aim: func () {
        ...
        if (getprop("/sim/current-view/name") == "Sensor View" and me.click_coord_cam != nil) {
            var (yaw, pitch, distance) = computer(coords_cam, me.click_coord_cam);
            me.update_cam(roll_deg, pitch_deg, yaw, pitch);
        }
    },
    ...
    _get_flir_auto_updater: func (offset) {
        return func (roll_deg, pitch_deg, yaw, pitch) {
            ...
            if (getprop("/sim/current-view/name") == "Sensor View") {
...
var list = func () {
    ...
    if (button == 1 or (getprop("controls/displays/cursor-click") and getprop("/sim/current-view/name") == "Sensor View")) {#LOCK
        gps = 0;
        if (lock_tgp) {
            lock_tgp = 0;
...
var fast_loop = func {
  var viewName = getprop("/sim/current-view/name"); 
    ...
    if (viewName == "Sensor View" and (getprop("gear/gear/wow"))) {
        ...
        # FLIR TGP stuff:
        setprop("aircraft/flir/target/view-enabled", viewName == "Sensor View");
        setprop("sim/rendering/als-filters/use-filtering", viewName == "Sensor View");
        setprop("sim/rendering/als-filters/use-IR-vision", viewName == "Sensor View" and ir);
    ...
    var dt = systime();
    if (viewName == "Sensor View" and getprop("f16/stores/tgp-mounted") and enable) {
